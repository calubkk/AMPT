...
class IntentDataset():
    ...
    def loadDataset(self, dataDirList,perc = 0.7):
        '''
        加载数据，返回utterance，label，还有每个数据对应的domain，就是不管什么域和dataname的问题，就给他们全部揉在一起
        '''
        ...
        if len(countSet) > 1:  #他是用了set的性质，来确保三者的长度都保持了一致
    ...
    def train_test_split(self,perc=0.7):
        ...
        ids = list(range(len(self.uttList)))
        num = int(len(self.uttList)*perc)
        train_ids = random.sample(ids, num) 
        test_ids = []
        for i in ids:
        ...
        testtoklist=[]
        for i in train_ids:
            traindomlist.append(self.domList[i])
            trainlablist.append(self.labList[i])
            trainuttlist.append(self.uttList[i])
            traintoklist.append(self.tokList[i])
        for i in test_ids:
            testdomlist.append(self.domList[i])
            testlablist.append(self.labList[i])
            testuttlist.append(self.uttList[i])
            testtoklist.append(self.tokList[i])
        
        trainDataset = IntentDataset(traindomlist, trainlablist, trainuttlist, traintoklist, multi_label=False)
        testDataset = IntentDataset(testdomlist, testlablist, testuttlist, testtoklist, multi_label=False)
    ...
    def splitDomain(self, domainName: list, multi_label=False):
        domList = self.getDomList() #这里面的domList在之前的loaddata里面已经计算出来了

        # collect index
        indList = []
        for ind, domain in enumerate(domList): #再根据定义的domain进行筛选
        ...
        if self.tokList:
            ...
            domTokList = []
        domDataset = IntentDataset(domDomList, domLabList, domUttList, domTokList, multi_label=multi_label)
