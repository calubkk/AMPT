...
class UniformTaskSampler(TaskSampler):
    ...
    def sampleClassIDsDataInd(self):
        ...
        #print(len(uniqueGlbLabIDs))
        # random sample global label IDs
        taskGlbLabIDs = random.sample(uniqueGlbLabIDs, self.way) #随机选取5个类
        taskInfo[META_TASK_GLB_LABID] = taskGlbLabIDs  #也就是说有这5个label_id
        ...
        for labID in taskGlbLabIDs: #遍历这5个任务
            # random sample support data and query data
            dataInds = random.sample(labID2DataInd[labID], self.shot+self.query) #随机的sample query+shot个data
    ...
    def collectDataForTask(self, taskDataInds):
        ...
        tokList = self.dataset.uttList
        labList = self.dataset.getLabID()
        domList = self.dataset.domList
        ...
        task[META_TASK_SHOT_DOM]       = [domList[i] for i in taskDataInds[META_TASK_SHOT_DATAIND]]
        ...
        task[META_TASK_QUERY_DOM]  = [domList[i] for i in taskDataInds[META_TASK_QUERY_DATAIND]]
